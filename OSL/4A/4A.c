...
void *producer(void *num)
{
	...
	while(1)
	{
		z=(rand()%10)+1;
		sleep(z);
		sem_wait(&b1.empty);// wait(empty)

		sem_getvalue(&b1.empty,&value);
		if(value==0)
			printf("\n***** Buffer Full. Producer %d is waiting for Consumer to Consume item. *****\n", i);

		pthread_mutex_lock(&b1.lock);// wait(mutex)


		printf("\nProducer %d produced %d item (|)", i,item);//adding data to buffer
		b1.buffer[(b1.in++)%buff_size]=item++;


		pthread_mutex_unlock(&b1.lock);	// signal(mutex)

		sem_post(&b1.full);// signal(full)
